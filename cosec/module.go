...
func (Module) Startup(ctx context.Context, mono monolith.Monolith) (err error) {
	...
	// ! we created streams for all the messages we intend to receive, including events, commands, and replies
	...
	// create the store and repository in our composition root
	sagaStore := pg.NewSagaStore("cosec.sagas", mono.DB(), reg)
	// The saga data generic is defined with a pointer so that it can be modified
	// by the functions we will be adding to the saga steps.
